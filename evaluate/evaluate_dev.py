"""
Dev evaluator — runs against the validation split only.

Measures:
  1. Query latency   — p50 / p95 across N trials per slot
  2. Australian rate — % of returned items with is_australian = TRUE
  3. Image coverage  — % of returned items with image_url not null/empty
  4. Slot correctness — % of items eligible for the requested meal slot

Results are written to:
    results/val_logs/eval_<timestamp>.json   (machine-readable)
    results/val_logs/eval_<timestamp>.txt    (human-readable table)

Run this freely during development. Never run evaluate_test.py until release.

Usage:
    python evaluate/evaluate_dev.py [--trials 30] [--top-k 10] [--prefetch 100]
"""
...
# Typical per-slot calorie targets used in production.
...
def _sample_text_queries(ds: LocalFoodDataset, val_ids: set[str], n: int, seed: int) -> list[str]:
    """Pull n food_name values from the val split to use as text queries."""
    rng = random.Random(seed)
    id_list = rng.sample(sorted(val_ids), min(n * 10, len(val_ids)))
    ...
    if not names:
        return ["chicken", "salad", "oats", "eggs", "pasta"]
    return names[:n]
...
def _static_coverage(ds: LocalFoodDataset, val_ids: set[str]) -> dict:
    """Count Australian-eligible val items and their image coverage without running search."""
    id_sample = sorted(val_ids)[:50000]  # cap for speed
...
def main() -> int:
    ...
    parser.add_argument("--trials", type=int, default=30, help="Searches per slot (default 30)")
    ...
    parser.add_argument(
        "--out-dir",
        default=str(ROOT / "results" / "val_logs"),
    )
    ...
    val_ids = _load_manifest(splits_dir, "val")
    print(f"Val split: {len(val_ids):,} RecipeIds")
    ...
    if not ds.is_ready:
        ...
        return 1
    print(f"Dataset ready: {ds.db_path}")

    rng = random.Random(args.seed)
    text_queries = _sample_text_queries(ds, val_ids, n=200, seed=args.seed)
    print(f"Sampled {len(text_queries)} text queries from val split.")

    print("\nRunning static coverage check…")
    static = _static_coverage(ds, val_ids)
    print(f"  val sample : {static['val_sample_size']:,}")
    ...
    txt_content = (
        f"GoodHealthMate v2 — VAL evaluation  {timestamp}\n"
        f"DB: {ds.db_path}\n"
        f"Trials/slot: {args.trials}  top_k: {args.top_k}  prefetch: {args.prefetch}\n\n"
        f"Static coverage (val sample {static['val_sample_size']:,} rows):\n"
